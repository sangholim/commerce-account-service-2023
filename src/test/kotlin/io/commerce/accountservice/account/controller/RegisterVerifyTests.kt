...
class RegisterVerifyTests(
    @MockkBean
    private val accountFacadeService: AccountFacadeService,
    private val webTestClient: WebTestClient
) : DescribeSpec({

    describe("회원 가입시 이메일 인증") {
        val email = AccountFixture.createUser
        val client = webTestClient
            .post().uri("/account/register/verify/email")
        ...
        describe("이메일 인증 번호 발송") {
            ...
            context("다른 계정의 이메일인 경우") {
                coEvery {
                    accountFacadeService.sendVerificationMessage(
                        VerificationItem.REGISTER,
    ...
    describe("회원 가입시 휴대폰 번호 인증") {
        val phoneNumber = AccountFixture.normalPhoneNumber
        val client = webTestClient
            .post().uri("/account/register/verify/sms")
            .contentType(MediaType.APPLICATION_JSON)
